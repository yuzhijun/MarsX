package com.winning.mars_security.util;

public class Constants {
    public static final String ACTION_BACKUP = "action_backup";
    public static final String ACTION_LOCK_SCREEN = "action_lock_screen";
    public static final String ACTION_RESET_PASSWORD = "action_reset_password";
    public static final String ACTION_DATA_ERASURE = "action_data_erasure";
    public static final String ACTION_WIPE_DATA = "action_wipe_data";
    public static final String APP_ALL = "app_all";
    public static final String ACTION_VIBRATE = "action_vibrate";
    public static final String ACTION_RING_DOWN = "action_ring_down";
    public static final int STRATEGY_SUCCESS = 1;
    public static final int STRATEGY_FAIL = 0;

}
